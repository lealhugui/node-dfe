...
class EnviaProcessor {
   ...
   * @param documento Array de documentos modelo 55 ou 1 documento modelo 65
   */
  async obterXmlLoteAssinado(documento) {
    var result = null;
  ...
  async transmitirXml(xmlLote, nfeObj) {
    ...
    try {
      ...
      console.log(retornoEnvio || "sem retorno do envio");
  ...
  gerarXml(documento) {
    ...
    let dadosChave = this.gerarChaveNF(
      this.configuracoes.empresa,
      documento.docFiscal
    );
    let NFe = {
      $: {
  ...
  gerarNFe(documento, dadosChave) {
    ...
    if (this.configuracoes.responsavelTecnico)
      nfe.infRespTec = this.getResponsavelTecnico(
        this.configuracoes.responsavelTecnico,
        dadosChave.chave
      );
    return nfe;
  }
  ...
  getDetImposto(imposto, modelo, cfop) {
    let detImposto = {
      ...
      COFINS: imposto.cofins
        ? [this.getImpostoCOFINS(imposto.cofins, modelo)]
        : [],
      PISST: imposto.pisst ? [this.getImpostoPISST(imposto.pisst)] : [],
      COFINSST: imposto.cofinsst
        ? [this.getImpostoCOFINSST(imposto.cofinsst)]
        : [],
  ...
  getCobr(cobranca) {
    if (
      cobranca &&
      cobranca.fatura &&
      (cobranca.fatura.nFatura ||
        cobranca.fatura.vOriginal > 0 ||
      ...
      cobr.fat.vDesc = (
        cobranca.fatura.vDesconto ? cobranca.fatura.vDesconto : 0
      ).toFixed(2);
