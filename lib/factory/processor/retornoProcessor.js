...
class RetornoProcessor {
  ...
  async executar(recibo, nfeObj) {
    ...
    try {
      ...
      let cStat = "105";
      let _tentativa = 0;
      const { tentativas, aguardarConsultaRetorno } =
        this.configuracoes.webservices;
      do {
        ...
        try {
          ...
          Utils.log(
            Utils.jsonOneLevel(retornoConsulta),
            "retornoConsulta.data"
          );
        } catch (e) {
          Utils.log(
            "retornoProcessor.envir: ja deu erro pra logar.......",
            "retornoConsulta"
          );
        }
      ...
      if (retornoConsulta) result = retornoConsulta;
    } catch (ex) {
