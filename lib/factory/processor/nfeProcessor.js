...
class NFeProcessor {
    ...
    async processarDocumento(documento) {
        ...
        try {
            ...
            let xmlAssinado = signature_1.Signature.signXmlX509(doc.xml, 'infNFe', this.empresa.certificado);
    ...
    configuraUrlsSefaz(modelo, ambiente) {
        if (!soapAutorizacao || !soapRetAutorizacao) {
            ...
            soapAutorizacao = Sefaz.getSoapInfo(this.empresa.endereco.uf, ambiente, nfe_1.ServicosSefaz.autorizacao);
            soapRetAutorizacao = Sefaz.getSoapInfo(this.empresa.endereco.uf, ambiente, nfe_1.ServicosSefaz.retAutorizacao);
        }
    }
    ...
    async consultarProc(xml, cert) {
        return await webserviceHelper_1.WebServiceHelper.makeSoapRequest(xml, cert, soapRetAutorizacao, this.webProxy);
    }
    async enviarNF(xml, cert) {
        return await webserviceHelper_1.WebServiceHelper.makeSoapRequest(xml, cert, soapAutorizacao, this.webProxy);
    }
    ...
    getIde(documento, dadosChave) {
        let ide = {
            ...
            indIntermed : documento.indIntermed ? Utils.getEnumByValue(schema.TNFeInfNFeIdeIndIntermed, documento.indIntermed) : null, // NT 2020.006
