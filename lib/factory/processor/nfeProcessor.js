 ...
 */
class NFeProcessor {
  constructor(empresa, responsavelTecnico, webProxy) {
  ...
  async processarDocumento(documento) {
    ...
    try {
      this.configuraUrlsSefaz(
        documento.docFiscal.modelo,
        documento.docFiscal.ambiente
      );
      let doc = this.gerarXml(documento);
      let xmlAssinado = signature_1.Signature.signXmlX509(
        doc.xml,
        "infNFe",
   ...
   * @param documento Array de documentos modelo 55 ou 1 documento modelo 65
   */
  async processarDocumentoAsync(documento) {
    let result = {
      success: false,
    };
    try {
  ...
  configuraUrlsSefaz(modelo, ambiente) {
    //if (!soapAutorizacao || !soapRetAutorizacao) {
    // nao da pra deixar condicionado a ja estar carregado porque senao so recarrega 1x no start da app e durante o uso se for
    // preciso alternar entre NFe ou NFC-e da erro por nao localizar a urlSefaz do modelo usado na 2a vez que alternar
    let Sefaz = modelo == "65" ? sefazNfce_1.SefazNFCe : sefazNfe_1.SefazNFe;
    soapAutorizacao = Sefaz.getSoapInfo(
      this.empresa.endereco.uf,
      ambiente,
      nfe_1.ServicosSefaz.autorizacao
    );
    soapRetAutorizacao = Sefaz.getSoapInfo(
      this.empresa.endereco.uf,
      ambiente,
      nfe_1.ServicosSefaz.retAutorizacao
    );
    //    }
  }
  appendQRCodeXML(documento, xmlAssinado) {
    let qrCode = null;
    let xmlAssinadoObj = xmlHelper_1.XmlHelper.deserializeXml(xmlAssinado, {
      explicitArray: false,
    });
    ...
    if (documento.docFiscal.isContingenciaOffline) {
      ...
      let digestValue =
        Object(xmlAssinadoObj).NFe.Signature.SignedInfo.Reference.DigestValue;
      qrCode = this.gerarQRCodeNFCeOffline(
        soapAutorizacao.urlQRCode,
        chave,
        "2",
        documento.docFiscal.ambiente,
        diaEmissao,
    ...
    let qrCodeXml = xmlHelper_1.XmlHelper.serializeXml(qrCodeObj, "infNFeSupl")
      .replace(">]]>", "]]>")
      .replace("<![CDATA[<", "<![CDATA[");
    return {
      qrCode: qrCodeObj,
  ...
  async transmitirXml(xmlLote, modelo, ambiente, nfeObj) {
    let result = {
      success: false,
      nfe: nfeObj,
    };
    try {
      let retEnviNFe = null;
      if (!nfeObj) {
        let xmlObj = xmlHelper_1.XmlHelper.deserializeXml(xmlLote, {
          explicitArray: false,
        });
      ...
      console.log(
        retEnviNFe &&
          ...
          retEnviNFe.protNFe.infProt.cStat == "100"
      );
      ...
      if (
        ...
        let cStat = "105";
        do {
          retornoConsulta = await this.consultarProc(
            xmlConRecNFe,
            this.empresa.certificado
          );
          try {
  ...
  async consultarProc(xml, cert) {
    return await webserviceHelper_1.WebServiceHelper.makeSoapRequest(
      xml,
      cert,
      soapRetAutorizacao,
      this.webProxy
  ...
  gerarXmlConsultaProc(ambiente, recibo) {
    let consulta = {
  ...
  gerarXml(documento) {
    ...
    let NFe = {
      ...
      infNFe:
        documento.docFiscal.modelo == "65"
          ? this.gerarNFCe(documento, dadosChave)
          : this.gerarNFe(documento, dadosChave),
    };
  ...
  gerarNFe(documento, dadosChave) {
    ...
    if (this.responsavelTecnico)
      nfe.infRespTec = this.getResponsavelTecnico(
  ...
  gerarNFCe(documento, dadosChave) {
    ...
    if (documento.destinatario)
      nfce.dest = this.getDest(
        documento.destinatario,
        documento.docFiscal.ambiente
      );
    nfce.det = this.getDet(
      documento.produtos,
      documento.docFiscal.ambiente,
      documento.docFiscal.modelo
    );
    ...
    if (this.responsavelTecnico)
      nfce.infRespTec = this.getResponsavelTecnico(
        this.responsavelTecnico,
        dadosChave.chave
      );
    return nfce;
  ...
  getIde(documento, dadosChave) {
    let ide = {
      ...
      tpNF: Utils.getEnumByValue(
        schema.TNFeInfNFeIdeTpNF,
        documento.tipoDocumentoFiscal
      ),
      idDest: Utils.getEnumByValue(
        schema.TNFeInfNFeIdeIdDest,
        documento.identificadorDestinoOperacao
      ),
      cMunFG: documento.codIbgeFatoGerador,
      tpImp: Utils.getEnumByValue(
        schema.TNFeInfNFeIdeTpImp,
        documento.tipoImpressao
      ),
      tpEmis: Utils.getEnumByValue(
        schema.TNFeInfNFeIdeTpEmis,
        documento.tipoEmissao
      ),
      cDV: dadosChave.dv,
      ...
      indFinal: Utils.getEnumByValue(
        schema.TNFeInfNFeIdeIndFinal,
        documento.indConsumidorFinal
      ),
      indPres: Utils.getEnumByValue(
        schema.TNFeInfNFeIdeIndPres,
        documento.indPresenca
      ),
      indIntermed: documento.indIntermed
        ? Utils.getEnumByValue(
            schema.TNFeInfNFeIdeIndIntermed,
            documento.indIntermed
          )
        : null, // NT 2020.006
    ...
    if (!documento.indIntermed) {
      delete ide.indIntermed; // NT 2020.006
    }
    return ide;
  }
  getEmit(empresa) {
  ...
  getDest(destinatario, ambiente) {
    let dest = {};
    if (destinatario.isEstrangeiro) dest.idEstrangeiro = destinatario.documento;
    if (destinatario.documento.length == 14) dest.CNPJ = destinatario.documento;
    else dest.CPF = destinatario.documento;
    dest.xNome =
      ambiente == "2"
    ...
    dest.indIEDest = Utils.getEnumByValue(
      schema.TNFeInfNFeDestIndIEDest,
      destinatario.indicadorIEDestinario
    );
    ...
    for (let i = 0; i < produtos.length; i++) {
      det_list.push({
        ...
        impostoDevol:
          ...
          produtos[i].prod.percentualDevolucao > 0
            ? this.getImpostoDevolucao({
                pDevol: produtos[i].prod.percentualDevolucao,
        ...
        case "00":
          result = {
            ICMS00: {
              orig: icms.orig,
              CST: icms.CST,
              modBC: Utils.getEnumByValue(
                schema.TNFeInfNFeDetImpostoICMSICMS00ModBC,
                icms.modBC
              ),
              vBC: icms.vBC,
              pICMS: icms.pICMS,
          ...
          if (
            ...
            result = {
              ICMS10: {
                ...
                CST: icms.CST,
                modBC: Utils.getEnumByValue(
                  schema.TNFeInfNFeDetImpostoICMSICMSPartModBC,
                  icms.modBC
                ),
                vBC: icms.vBC,
                pRedBC: icms.pRedBC,
                pICMS: icms.pICMS,
                vICMS: icms.vICMS,
                modBCST: Utils.getEnumByValue(
                  schema.TNFeInfNFeDetImpostoICMSICMSPartModBCST,
                  icms.modBCST
                ),
                pMVAST: icms.pMVAST,
            ...
            result = {
              ICMS10: {
                ...
                CST: icms.CST,
                modBC: Utils.getEnumByValue(
                  schema.TNFeInfNFeDetImpostoICMSICMS10ModBC,
                  icms.modBC
                ),
                pICMS: icms.pICMS,
                vBC: icms.vBC,
        ...
        case "20":
          result = {
            ICMS20: {
              ...
              CST: icms.CST,
              modBC: Utils.getEnumByValue(
                schema.TNFeInfNFeDetImpostoICMSICMS20ModBC,
                icms.modBC
              ),
              pRedBC: icms.pRedBC,
              vBC: icms.vBC,
              ...
              motDesICMS: Utils.getEnumByValue(
                schema.TNFeInfNFeDetImpostoICMSICMS20MotDesICMS,
                icms.motDesICMS
              ),
            },
          };
        ...
        case "30":
          result = {
            ICMS30: {
              ...
              vICMSST: icms.vICMSST,
              motDesICMS: Utils.getEnumByValue(
                schema.TNFeInfNFeDetImpostoICMSICMS30MotDesICMS,
                icms.motDesICMS
              ),
              vICMSDeson: icms.vICMSDeson,
              vBCFCPST: icms.vBCFCPST,
        ...
        case "50":
          result = {
            ICMS40: {
              ...
              CST: icms.CST,
              motDesICMS: Utils.getEnumByValue(
                schema.TNFeInfNFeDetImpostoICMSICMS40MotDesICMS,
                icms.motDesICMS
              ),
              vICMSDeson: icms.vICMSDeson,
            },
        ...
        case "51":
          result = {
            ICMS51: {
              ...
              CST: icms.CST,
              modBC: Utils.getEnumByValue(
                schema.TNFeInfNFeDetImpostoICMSICMS51ModBC,
                icms.modBC
              ),
              vBC: icms.vBC,
              pRedBC: icms.pRedBC,
          ...
          if (
            ...
            result = {
              ICMS90: {
                ...
                CST: icms.CST,
                modBC: Utils.getEnumByValue(
                  schema.TNFeInfNFeDetImpostoICMSICMS90ModBC,
                  icms.modBC
                ),
                vBC: icms.vBC,
                pRedBC: icms.pRedBC,
                ...
                vICMSST: icms.vICMSST,
                motDesICMS: Utils.getEnumByValue(
                  schema.TNFeInfNFeDetImpostoICMSICMS90MotDesICMS,
                  icms.motDesICMS
                ),
                vICMSDeson: icms.vICMSDeson,
                vBCFCP: icms.vBCFCP,
  ...
  getImpostoCOFINS(cofins, modelo) {
    ...
    switch (cofins.CST) {
      ...
      case "99":
        if (
          cofins.vBC + cofins.pCOFINS > 0 &&
          cofins.qBCProd + cofins.vAliqProd > 0
        )
  ...
  getImpostoCOFINSST(COFINSST) {
    ...
    if (
      ...
      if (
        COFINSST.vBC + COFINSST.pCOFINS > 0 &&
        COFINSST.qBCProd + COFINSST.vAliqProd > 0
      )
        throw "As TAG <vBC> e <pCOFINS> não podem ser informadas em conjunto com as TAG <qBCProd> e <vAliqProd>";
  ...
  getDetalhamentoPagamentos(pagamentos) {
    ...
    for (const pag of pagamentos) {
      ...
      detPag.indPag = Utils.getEnumByValue(
        ...
        pag.indicadorFormaPagamento
      );
      detPag.tPag = Utils.getEnumByValue(
  ...
  getDetalhamentoCartao(dadosCartao) {
    return {
      tpIntegra: Utils.getEnumByValue(
        schema.TNFeInfNFePagDetPagCardTpIntegra,
        dadosCartao.tipoIntegracao
      ),
      CNPJ: dadosCartao.cnpj,
      tBand: Utils.getEnumByValue(
        schema.TNFeInfNFePagDetPagCardTBand,
