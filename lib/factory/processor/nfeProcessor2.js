...
class NFeProcessor {
  constructor(configuracoes) {
    ...
    this.retornoProcessor = new retornoProcessor_1.RetornoProcessor(
      this.configuracoes
    );
    this.enviaProcessor = new enviaProcessor_1.EnviaProcessor(
      this.configuracoes
    );
    this.eventoProcessor = new eventoProcessor_1.EventoProcessor(
      this.configuracoes
    );
    this.inutlizacaoProcessor = new inutilizaProcessor_1.InutilizaProcessor(
      this.configuracoes
    );
  }
  /**
  ...
  async executar(documento) {
    ...
    try {
      ...
      if (result.envioNF && result.envioNF.data) {
        ...
        if (arquivos && arquivos.salvar) {
          ...
          if (result.success == true && retConsReciNFe.cStat == "104") {
            ...
            const nfe_enviada = Object(
              xmlHelper_1.XmlHelper.deserializeXml(result.envioNF.xml_enviado, {
                explicitArray: false,
              })
            );
            const nfeProc = {
            ...
            let xmlNfeProc = xmlHelper_1.XmlHelper.serializeXml(
              nfeProc,
              "nfeProc"
            );
            ...
            fs.writeFileSync(
              filenameConsultaEnvio,
              result.consultaProc.xml_enviado
            );
            fs.writeFileSync(
              filenameConsultaRetorno,
              result.consultaProc.xml_recebido
            );
          }
        }
        ...
        console.error(
          "nfeProcessor.executar: Erro ao realizar requisição",
          result
        );
  ...
  async obterXmlLoteAssinado(documento) {
    const { arquivos, geral } = this.configuracoes;
    var result = await this.enviaProcessor.obterXmlLoteAssinado(documento);
    return result;
  }
}
